...
const CANONICAL_TZ_OUTPUT = path.join(
  process.cwd(),
  "src",
  "utils",
  "canonicalTzRegions.generated.ts",
);
...
async function generateGlobeData(): Promise<void> {
  console.log(
    "🌍 Generating unified globe data (Natural Earth + World 110m)...\n",
  );
  ...
  try {
    ...
    const processedTzFeatures = timezonesGeo.features.map((f) => {
      ...
      // Prefer canonical IANA name from tz_name1st when present.
      // Fall back to UTC format labels and then display name.
      const tzid = (p.tz_name1st ?? p.time_zone ?? p.name) as string;
    ...
    const fileContent = `// Generated by scripts/update-globe-data.ts
...
`;
    ...
    console.log(
      `  ✅ Generated: ${CANONICAL_TZ_OUTPUT} (${sortedRegions.length} regions)`,
    );
